  ...
  private def configError(
    ...
    origin
      .map(o => {
        ...
        new AkkaException.ExceptionSource("Configuration error", message, e.orNull) {
      ...
      .getOrElse(new AkkaException("Configuration error", message, e.orNull))
  }
...
case class Configuration(underlying: Config) {
   ...
   */
  def reportError(path: String, message: String, e: Option[Throwable] = None): AkkaException = {
   ...
   */
  def getDeprecatedWithFallback(path: String, deprecated: String): Configuration = {
   ...
   * @return a configuration exception
   */
  def globalError(message: String, e: Option[Throwable] = None): AkkaException = {
